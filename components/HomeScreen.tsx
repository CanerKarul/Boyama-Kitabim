...
const HomeScreen: React.FC<HomeScreenProps> = ({ onGenerate }) => {
  ...
  const handleSubmit = () => {
    ...
    setNameError('');
    onGenerate({ name: name.trim(), pageCount, theme: theme.trim() || 'Genel', age, canWrite });
  };
    ...
    <>
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 dark:border-slate-800 px-4 sm:px-10 py-4">
        <div className="flex items-center gap-4 text-slate-800 dark:text-white">
          ...
          <h2 className="font-display text-text-light dark:text-text-dark text-lg font-bold leading-tight tracking-[-0.015em]">Coloring Book Creator</h2>
        </div>
        ...
        <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
          <div className="flex flex-col items-center gap-8 sm:gap-10">
            <div className="flex flex-col gap-4 text-center px-4">
              <p aria-live="polite" className="font-bold text-sm text-primary">1/3 Ana Ekran</p>
              <h1 className="font-display text-slate-900 dark:text-white text-4xl sm:text-5xl font-black leading-tight tracking-[-0.03em]">Kişisel Boyama Kitabını Saniyeler İçinde Oluştur</h1>
              <p className="text-slate-600 dark:text-slate-400 text-base sm:text-lg leading-relaxed max-w-2xl mx-auto">
                Çocuğunun adını ve hayalindeki temayı gir, sihirli bir şekilde boyama sayfalarını hazırlayalım.
              </p>
            </div>
            <div className="w-full max-w-xl mx-auto flex flex-col gap-6 sm:gap-8">
              <div className="flex flex-col">
                <label className="flex flex-col w-full">
                  <p className="text-slate-800 dark:text-slate-200 text-base font-medium leading-normal pb-2">Çocuğun Adı</p>
                  <input value={name} onChange={(e) => { setName(e.target.value); if (e.target.value) setNameError(''); }} aria-describedby="name-error" aria-label="Çocuğun Adı" className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded text-slate-800 dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary/50 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:border-primary h-14 placeholder:text-slate-400 dark:placeholder-slate-500 p-[15px] text-base font-normal leading-normal ring-2 ring-transparent" placeholder="Örn: Ayşe" />
                </label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="w-full">
                    ...
                    <div className="flex w-full shrink-[3] items-center justify-between">
                        <p className="text-slate-800 dark:text-slate-200 text-base font-medium leading-normal">Çocuğun Yaşı</p>
                 ...
                 <div className="w-full flex items-center justify-center">
                    <label className="flex items-center gap-3 cursor-pointer">
                        <p className="text-slate-800 dark:text-slate-200 text-base font-medium">Okuma yazma biliyor mu?</p>
              ...
              <div className="w-full">
                <div className="relative flex w-full flex-col items-start justify-between gap-3">
                  <div className="flex w-full shrink-[3] items-center justify-between">
                    <p className="text-slate-800 dark:text-slate-200 text-base font-medium leading-normal">Sayfa Sayısı</p>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm font-normal pt-1">En az 1, en fazla 10 sayfa seçilebilir.</p>
                </div>
              </div>
              <div className="flex flex-col gap-6 p-4 sm:p-6 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
                <label className="flex flex-col w-full">
                  <p className="text-slate-800 dark:text-slate-200 text-base font-medium leading-normal pb-2">Boyama Teması</p>
                  <input value={theme} onChange={handleThemeInputChange} aria-label="Boyama Teması" className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded text-slate-800 dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary/50 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:border-primary h-14 placeholder:text-slate-400 dark:placeholder-slate-500 p-[15px] text-base font-normal leading-normal" placeholder="Hayalindeki temayı yaz (örn: uzaydaki sevimli kediler)" />
                </label>
              </div>
              <div className="flex flex-col gap-4">
                <p className="text-slate-800 dark:text-slate-200 text-base font-medium leading-normal text-center">Veya Popüler Temalardan Seç:</p>
            ...
            <button onClick={handleSubmit} aria-label="Boyama Kitabı Oluştur ve Sonraki Adıma Geç" className="group flex min-w-[84px] w-full max-w-xl cursor-pointer items-center justify-center overflow-hidden rounded-lg h-14 px-6 bg-accent text-white text-base font-bold leading-normal tracking-[0.015em] shadow-lg shadow-accent/30 hover:bg-opacity-90 transition-all duration-300 ease-in-out transform hover:scale-105">
              <span className="truncate">Boyama Kitabı Oluştur</span>
